...
void DecodeTokens(void){
	...
	for (ucTokenCounter = 0; ucTokenCounter < ucTokenNr; ucTokenCounter++){
		...
		if (OK == eStringToKeyword(psCurrentToken->uValue.pcString, &eKeywordCode)){
			psCurrentToken->eType = KEYWORD;
			psCurrentToken->uValue.eKeyword = eKeywordCode;
		}
		else if (OK == eHexStringToUInt(psCurrentToken->uValue.pcString, &uiNumberValue)){
			psCurrentToken->eType = NUMBER;
			psCurrentToken->uValue.uiNumber = uiNumberValue;
